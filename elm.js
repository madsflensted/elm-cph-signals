...
Elm.Center.make = function (_elm) {
   ...
   var markdown = F2(function (width,string) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("content"),style(width)]),
      _U.list([$Markdown.toHtml(string)]));
   });
   return _elm.Center.values = {_op: _op
                               ,markdown: markdown
                               ,style: style};
};
...
Elm.Main.make = function (_elm) {
   ...
   var addNoneEffect = function (model) {
      return {ctor: "_Tuple2",_0: model,_1: $Effects.none};
   };
   _op["->>"] = F4(function (f,g,a,b) {    return g(A2(f,a,b));});
   var app = $StartApp.start({init: addNoneEffect(initialModel)
                             ,view: view
                             ,update: A2(_op["->>"],update,addNoneEffect)
                             ,inputs: inputs});
   var main = app.html;
